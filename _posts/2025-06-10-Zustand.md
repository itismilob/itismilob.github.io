...
create 함수의 **타입**
`{ts}function create<T>(stateCreatorFn: StateCreator<T>): UseBoundStore<StoreApi<T>>` 
...
더 정확한 타입 ([[커링 패턴]])
`{ts}create<T>()(stateCreatorFn: StateCreator<T, [], []>): UseBoundStore<StoreApi<T>>`
...
set 함수의 **타입**
`{ts}set: (partial: Partial<State> | (state: State) => Partial<State>, replace?: boolean) => void` 

**partial**은 상태 객체나 상태를 변경하는 함수를 받는다. (useState와 방식이 동일함)
- 직접 상태 객체 입력 : `{ts}set({count: 5});`
- 이전 상태를 기반으로 새 객체 반환 : `{ts}set(state => ({count: state.count + 1}));`
...
- `name` : 필수, 저장소의 키 이름
- `storage` : 저장소 선택 (기본값 : `{ts}createJSONStorage(()=>localStorage)`) -> MMKV 사용
